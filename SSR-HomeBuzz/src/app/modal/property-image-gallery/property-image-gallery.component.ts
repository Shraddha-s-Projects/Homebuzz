import { Component, OnInit, ViewContainerRef, ViewChild, Input, EventEmitter, Output, OnDestroy, Inject } from "@angular/core";
...
export class PropertyImageGalleryComponent implements OnInit, OnDestroy {
  ...
  tabs: TabList[] = [
    ...
    { Id: 'PropertyOverView', Name: 'Propery overview' },
  ...
  constructor(@Inject(WINDOW) private window: Window, @Inject(LOCAL_STORAGE) private localStorage: any, public dialogRef: MatDialogRef<PropertyImageGalleryComponent>,
  ...
  createMapImageAt45(divId: string) {
    ...
    google.maps.event.addListenerOnce(map, "tilesloaded", function () {
      if ($('.gm-style').length > 1) {
        drawDOM(document.querySelector('#divId'))
          .then((group: Group) => {
            // console.log(group);
  ...
  onShareOptionClick() {
    // window.history.pushState(null, null, 'property/share/' + this.getRandom());
...
// var imagePath = document.getElementById("divId");
