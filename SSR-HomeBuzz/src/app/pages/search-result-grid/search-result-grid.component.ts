import { Component, OnInit, Input, NgZone, Output, EventEmitter, OnDestroy, Inject } from '@angular/core';
...
export class SearchResultGridComponent implements OnInit, OnDestroy {
  ...
  onScroll(value: number): void {
    // let scrollOffset1: HTMLElement = document.getElementById('address-grid');
    // scrollOffset1.style.pix
    // if(scrollOffset){
    //   scrollOffset.hei
    // }
    ...
    let el: HTMLCollectionOf<Element> = document.getElementsByClassName('agent-option');
    if (el) {
    ...
    this._unsubscribeAll.add(this.searchResultPageService.updateAgentOption(Obj).subscribe((data: any) => {
      if (data.Success) {
        ...
        if (index > -1) {
          ...
          // if (this.properties[index].IsAgentListProperty) {
          ...
          // if (data.Model.AgentOptionId == 1) {
          //   this.properties[index].IsAgentListProperty = true;
          //   this.properties[index].AgentOptionArr = this.properties[index].AgentOptionArr.filter(t => t.Id != 1);
          // }
