...
--select 
--distinct
--(select COUNT(Id) from PropertyData where Address LIKE '%'+ @SearchTerm + '%')TotalProperties,
--(select ROUND(AVG(HomebuzzEstimate), 0) from PropertyData where Address LIKE '%'+ @SearchTerm + '%') AverageEstimatePrice,
--(select COUNT(pc4.Id) from PropertyCrudData pc4 INNER JOIN PropertyDetail pd4 ON pc4.PropertyDetailId = pd4.Id where Address LIKE '%'+ @SearchTerm + '%')ActiveProperties,
--(select COUNT(pd1.Id) from PropertyDetail pd1 
--INNER JOIN PropertyCrudData pc1 on pd1.Id = pc1.PropertyDetailId  where pd1.Status = 'Not listed' 
--AND  pc1.Address LIKE '%'+ @SearchTerm + '%')ViewedProperties,
--(select COUNT(pd2.Id) from PropertyDetail  pd2 
--INNER JOIN PropertyCrudData pc2 on pd2.Id = pc2.PropertyDetailId where pd2.Status = 'Pre-market' 
--AND pc2.Address LIKE '%'+ @SearchTerm + '%')ClaimedProperties,
--(select COUNT(pd3.Id) from PropertyDetail pd3 
--INNER JOIN PropertyCrudData pc3 on pd3.Id = pc3.PropertyDetailId where pd3.Status = 'For sale' 
--AND pc3.Address LIKE '%'+ @SearchTerm + '%')ForSaleProperties
END